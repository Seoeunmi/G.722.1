...
void CATEGORY(int *bit, float *rms_index, int Category[][14])
{
	/* STEP 1 & 2 */
	...
	/* STEP 0 */
	number_of_available_bits = 480 - sum_bit - 4 - 5;
...
void SQVH(float *mlt, float *rms, float *Q_rms, int Category[][14], int *number_of_region_bits, int *bit, int *select_categorization, int vector_index[][10], int k[][20])
{
	...
	for (int m = 0; m < 16; m++)
	{
		for (int r = 0; r < 14; r++)
		{
			if (Category[m][r] == 7) continue;

			for (int i = 0; i < 20; i++)
			{
				k[r][i] = MIN((int)(fabs(mlt[20 * r + i]) / (stepsize[Category[m][r]] * Q_rms[r]) + deadzone_rounding[Category[m][r]]), kmax[Category[m][r]]);
			}
...
void BITSTREAM(float *rms_index, int *number_of_region_bits, int *bit, int *code, int *select_categorization, int vector_index[][10], int Category[][14], int k[][20], float *mlt, FILE *(*fout2))
{
	...
	short count = 0;
	short buffer, save;
	int  n = 1, vector_bit, k_bit_num, total_count = 0, k_bit, vector_code;
	int vpr[7] = { 10, 10, 10, 5, 5, 4, 4 };
	int vd[7] = { 2, 2, 2, 4, 4, 5, 5 };

	//amplitude envelope bits
	total_count = 0;
	for (int i = 0; i < 14; i++)
	{
		k_bit_num = 0;
	...
	for (int r = 0; r < 14; r++)
	{
		if (Category[*select_categorization][r] == 7) continue;
		...
		for (int m = 0; m < vpr[Category[*select_categorization][r]]; m++)
		{
			...
			vector_code = MLT_SQVH_CODE(Category[*select_categorization][r], vector_index[r][m]);

			n = 1;
			for (int j = 1; j <= vector_bit; j++)
			{
				buffer = vector_code;
				buffer = (vector_code >> vector_bit - j);
				n++;
				buffer = buffer & 1;
				bit_stream |= buffer;
				count++;
				...
				if (count == 16)
				{
					...
					{
						buffer = (vector_code >> vector_bit - a);
						buffer = buffer & 1;
						bit_stream |= buffer;
						count++;
						bit_stream = (bit_stream << 1);
			...
			/* k가 0이 아닐때의 부호 보내기*/
			for (int i = 0; i < vd[Category[*select_categorization][r]]; i++)
			{
				if (k[r][i + k_bit_num] != 0)
				{
					if (mlt[20 * r + (i + k_bit_num)] > 0)
					{
						k_bit = 1;
						buffer = k_bit;
						buffer = buffer & 1;
			...
			k_bit_num = k_bit_num + vd[Category[*select_categorization][r]];
		}
	}
